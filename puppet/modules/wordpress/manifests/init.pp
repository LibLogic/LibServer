...
class wordpress::install {
  ...
  exec { 'wordpress-install':
    ...
    cwd => '/vagrant/',
    require => [File['/vagrant/composer.json'], Exec['comp-global']],		
  }

  file { '/vagrant/composer.json':
    source  => 'puppet:///modules/wordpress/json.conf',
    owner => 'vagrant',
  }
...
#  }
  ...
  file { '/vagrant/wp/wp-tests-config.php':
